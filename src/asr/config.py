...
def __process_dataset_manifests(
    datasets: Sequence[STDataset], dataset_names: list[str], root_location: Path, encodings_folder: Path
) -> None:
    for dataset, dataset_name in zip(datasets, dataset_names):
        ...
        for path, sentence, translation, speaker_id, sample_id in iterate_over_dataset(
            dataset, desc=f'Processing {dataset_name} to manifest'
        ):
...
def __process_dataset_config(
    root_location: Path, spm_filename: str, type: Literal['wav2vec'] | Literal['mel'] = 'mel'
) -> None:
