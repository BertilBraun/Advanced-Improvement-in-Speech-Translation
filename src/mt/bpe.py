...
class BPE:
    ...
    def encode_file(self, input_file: str, output_file: str) -> None:
        with open(output_file, "w", encoding="utf-8") as f_out:
            ...
            for line in tqdm(process_lines(input_file), desc=f"Segmenting '{file_name}' Dataset"):
    ...
    def decode_file(self, input_file: str, output_file: str) -> None:
        with open(output_file, "w", encoding="utf-8") as f_out:
            file_name = input_file.split("/")[-1]
            for line in tqdm(process_lines(input_file), desc=f"Desegmenting '{file_name}' Dataset"):
