...
class BPE:
    ...
    def __init__(
        self, retrain_spm: bool, model_file: Path, train_files: list[str] | None = None, vocab_size=10000
    ) -> None:
    ...
    def encode_lines(self, lines: list[str]) -> list[str]:
        return [' '.join(self._encode(line.strip())) for line in tqdm(lines, desc='Encoding Dataset')]

    def decode_lines(self, lines: list[str]) -> list[str]:
        return [self._decode(line.strip().split(' ')) for line in tqdm(lines, desc='Decoding Dataset')]
