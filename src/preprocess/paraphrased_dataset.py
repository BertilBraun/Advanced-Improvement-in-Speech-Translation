...
class ParaphrasedDataset(MTDataset):
    ...
    def _load_data(self) -> list[TextSample]:
        if not os.path.exists(COVOST_MT_PARAPHRASED_EN_FILE) or not os.path.exists(COVOST_MT_PARAPHRASED_DE_FILE):
            logger.error(
                f'Paraphrased files not found at {COVOST_MT_PARAPHRASED_EN_FILE} and {COVOST_MT_PARAPHRASED_DE_FILE}. Please run the paraphrase generation script.'
            )
            return self.base_dataset._data

        with open(COVOST_MT_PARAPHRASED_EN_FILE, 'r', encoding='utf-8') as en_file, open(
            COVOST_MT_PARAPHRASED_DE_FILE, 'r', encoding='utf-8'
        ) as de_file:
